...
void Display(void)
{
    ...
    glLoadIdentity();
	camera.direction = getForward(euler);
	...
	euler.pitch = 0;
	euler.roll = 0;
	euler.orientation.s = 0;
	euler.orientation.i = camera.position.x;
	euler.orientation.j = camera.position.y;
	euler.orientation.k = camera.position.z;
	*/

	QUATERNION aux = QuaternionFromAngleAxis(90, camera.position);
	VECTOR3D uup;
	uup.x = aux.i;
	uup.y = aux.j;
	uup.z = aux.k;

	//VECTOR3D forw = getForward(euler);

	//printf("aaaaaa%f \n", uup.z);
	//printf("bbbbbbbb%f \n", camera.up.z);

	//MATRIX4 lookAtMatrix = lookAt(camera.position, getForward(euler), uup);
	VECTOR3D a;
	VECTOR3D b;
	VECTOR3D c;
	a.x = 0; 
	...
	b.z = 0;
	c.z = 0;
	//MATRIX4 lookAtMatrix = lookAt(a, b, c); 
...
void Render(void)
{
	...
	punto.y = 3;
	punto.z = -5;
	drawDot(punto, 0.5,red);
	punto2.x = 1;
	punto2.y = 1;
	punto2.z = 5;
	drawDot(punto2, 0.5, blue);
	punto3=Add(punto, punto2);
	//punto3=Substract(punto, punto2);
	//punto3=Scale(punto, punto2);
	//punto3=MultiplyWithScalar(escalar, punto2);
	/*Magnitude(punto, punto2);
	Normalize(punto, punto2);
	CrossProduct(punto, punto2);
	DotProduct(punto, punto2);
	Transform(punto, punto2);
	Transpose(punto, punto2);
	*/
	drawDot(punto3, 0.5, green);
